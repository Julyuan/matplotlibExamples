...
save_addr = r"./test graph/"
addr1 = r"./test graph/openssl result.xlsx"
addr2 = r"./test graph/openssl result1.xlsx"
...
def symmetricResult():
    ...
    plt.plot(x1, y1, label="AES")
    plt.legend(bbox_to_anchor=(0., 1.02, 1., .102), loc=3,
               ncol=3, mode='expand', borderaxespad=0.)
    plt.xlabel("data size/byte")
    plt.ylabel("speed/(op/s)")
    plt.show()
    plt.savefig(save_addr + r"symmetric1", dpi=1000);

    x1a, y1a = dataSmooth(data1[0][:end], dataTransform(data1[1][:end]))
    plt.plot(x1a, y1a, label="AES")
    plt.legend(bbox_to_anchor=(0., 1.02, 1., .102), loc=3,
               ncol=3, mode='expand', borderaxespad=0.)
    plt.xlabel("data size/byte")
    plt.ylabel("time cost/ms")
    plt.show()
    plt.savefig(save_addr + r"symmetric2", dpi=1000);

    x1b, y1b = dataSmooth(data1[0][:end], multiElementWise(data1[0][:end], data1[1][:end]))
    plt.plot(x1b, y1b, label="AES")
    plt.legend(bbox_to_anchor=(0., 1.02, 1., .102), loc=3,
               ncol=3, mode='expand', borderaxespad=0.)
    plt.xlabel("data size/byte")
    plt.ylabel("bandwidth/(MB/s)")
    plt.show()
    plt.savefig(save_addr + r"symmetric3", dpi=1000);
    ...
    plt.ylabel("speed/(op/s)")
    plt.show()
    plt.savefig(save_addr + r"symmetric4", dpi=1000);
    ...
    plt.ylabel("time cost/ms")
    plt.show()
    plt.savefig(save_addr + r"symmetric5", dpi=1000);
    ...
    plt.ylabel("bandwidth/(MB/s)")
    plt.show()
    plt.savefig(save_addr + r"symmetric6", dpi=1000);
